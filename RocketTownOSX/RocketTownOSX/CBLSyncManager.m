...
NSString* const SyncManagerStateChangedNotification = @"SyncManagerStateChangeNotification";
...
- (void)replicationProgress:(NSNotificationCenter *)n {
  ...
  if (active != _active || completed != _completed || total != _total || status != _status
    ...
    if(status == kCBLReplicationOffline) {
      NSLog(@"Replication Offline");
    }
    if(status == kCBLReplicationIdle) {
      NSLog(@"Replication Idle");
    }
    if(status == kCBLReplicationActive) {
      NSLog(@"Replication Active");
    }
    [[NSNotificationCenter defaultCenter] postNotificationName: SyncManagerStateChangedNotification object: self];
  }
}
...
#pragma mark - CBLFacebookAuthenticator
